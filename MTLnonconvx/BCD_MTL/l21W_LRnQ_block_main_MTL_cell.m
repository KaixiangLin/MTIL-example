...
function [W,Q,B,q,F_val] = l21W_LRnQ_block_main_MTL_cell(X, Y, W, B, q,  parameters)
...
while true
    ...
    [W_new, funcval] = argmin_l21W_sparsa_MTL_cell(X, Y, W, q, B, d, K, r, rho1,FISTA_options);
    
 
    
    [q_new, funcval] = argmin_q_sparsa_MTL_cell(X, Y, W_new, q,     B, d, K, r, FISTA_options);
 
    
    [B_new, funcval] = argmin_B_sparsa_MTL_cell(X, Y, W_new, q_new, B, d, K, r, FISTA_options);
...
%  end
 ...
 f = smooth_funcvalueNcv(X,Y,W,q,B,d,K,r);
 f = f + norm(W(:),1)*rho1;
end
