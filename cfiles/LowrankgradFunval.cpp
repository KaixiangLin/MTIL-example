...
//     //testvariable
//     size_t mt;
//     mt  = mxGetM(mxGetCell(prhs[0],1));  // mt, row numbers    
//     plhs[3] = mxCreateDoubleMatrix(d, d, mxREAL);

    
    double *grad_W, *grad_Q;  // set gradient of W, B, Q.
    grad_W  = mxGetPr(plhs[0]);
    grad_Q  = mxGetPr(plhs[1]);
    
//     //testvariable
//     double *testXtj;
//     testXtj = mxGetPr(plhs[3]); 
...
//             Qt[ii] = Q1[ii ] + Q2[ii ] + Q3[ii ];
        }
        
//         if (icell == 2)
//         {
//             for (int kk = 0;kk<d*d;kk++)
//             {
//                 testXtj[kk] = grad_Q[kk]; // # checked
//             }
//         }
        ...
        for(int nsamp = 0; nsamp < mt; nsamp++)
        {
            ...
            // Xtemp = (Xtj*Qt*Xtj' + ZZ);
             ...
             for (int ii =0;ii<d; ii++)
             {
                 ...
                 {
                     grad_Q[ii+jj*d+icelld] += Xtj[ii]*Xtj_scale[jj];
                 }
             }
